...
public class MUtil
{
	...
	/**
	 * This checks and returns if the current block is touching the right the
	 * block ID specified
	 * 
	 * @param The
	 *            world
	 ...
	 * @param Block
	 *            z
	 * @param BlockIdtouching
	 *            - What block to check if touching?
	 * @return
	 */
	...
	/**
	 * This checks and returns if the current block is touching the left the
	 * block ID specified
	 * 
	 * @param The
	 *            world
			...
			if (recipeResult != null)
			{
				if (recipeResult == resultItem && recipeResult.getItemDamage() == resultItem.getItemDamage())
				{
					...
					scan--; // list is shifted after remove! Adjust index, so
							// next time we will check this value again.
				}
			}
		}
	}
