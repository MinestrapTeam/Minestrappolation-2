...
public class BlockPlutoniumOre extends Block
{
	...
	public void updateTick(World world, int i, int j, int k, Random random)
	{
		...
		AxisAlignedBB axisalignedbb = this.getCollisionBoundingBoxFromPool(world, i, j, k).expand(1.5, 1.5, 1.5);
		...
		else
		{
			while (var6.hasNext())
			{
				...
				if (EOConfig.shouldOresEffect)
				{
					...
					this.living.addPotionEffect(new PotionEffect(Potion.wither.getId(), 20, 1, false));
		...
		else
		{
			while (var8.hasNext())
			{
				...
				if (EOConfig.shouldOresEffect)
				{
					...
					this.skeleton.addPotionEffect(new PotionEffect(Potion.resistance.getId(), 180, 0, false));
					this.skeleton.addPotionEffect(new PotionEffect(Potion.damageBoost.getId(), 180, 0, false));
	...
	{
		// 1 is the XP it drops
		this.dropXpOnBlockBreak(par1World, par2, par3, par4, this.rand.nextInt(6) + 1);
	}
}